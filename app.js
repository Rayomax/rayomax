...
module.exports = async client => {
  app.get("/robots.txt", function(req, res) {
    ...
    res.send(`Sitemap: https://discordspacelist.ga/sitemap.xml`);
  });

  app.get("/sitemap.xml", async function(req, res) {
    let link = "<url><loc>https://discordspacelist.ga/</loc></url>";
    let botdataforxml = await botsdata.find();
    botdataforxml.forEach(bot => {
      link +=
        "\n<url><loc>https://discordspacelist.ga/bot/" +
        bot.botID +
        "</loc></url>";
    });
  ...
  const checkMaintence = async (req, res, next) => {
    ...
    if (d) {
      if (req.isAuthenticated()) {
        let uyevarmikiaq = client.guilds.cache
          .get(settingsc.serverID)
          .members.cache.get(req.user.id);
        if (uyevarmikiaq) {
          if (uyevarmikiaq.roles.cache.get(roles.websitedeveloper)) {
            next();
          } else {
            res.redirect(
              "/error?code=200&message=Our website is temporarily unavailable."
            );
          }
        } else {
          res.redirect(
    ...
    async (req, res) => {
      ...
      if (banned) {
        client.users.fetch(req.user.id).then(async a => {
          client.channels.cache.get(channels.login).send(
            new Discord.MessageEmbed()
              ...
              .setColor("RED")
              .setDescription(
                `[**${a.username}**#${a.discriminator}](https://discordspacelist.ga/user/${a.id}) user ** tried to login to the site ** but could not login because it was blocked from the site.`
              )
        ...
        req.session.destroy(() => {
          res.json({
            login: false,
            message: "You have been blocked from OP.",
            reason: `${banned.sebep}`
          });
          req.logout();
        });
        ...
        client.users.fetch(req.user.id).then(async a => {
          client.channels.cache.get(channels.login).send(
            new Discord.MessageEmbed()
              ...
              .setColor("GREEN")
              .setDescription(
                `[**${a.username}**#${a.discriminator}](https://discordspacelist.ga/user/${a.id}) user ** to the site** logged in.`
              )
              .addField("Username", a.username)
              .addField("User ID", a.id)
              .addField("User Discriminator", a.discriminator)
  ...
  app.get("/dc", (req, res) => {
    res.redirect("https://discord.gg/p6ExKQSgQG");
  });
  ...
  app.get("/uptime/links", checkMaintence, checkAuth, async (req, res) => {
    ...
    renderTemplate(res, req, "uptime/mylinks.ejs", {
      req,
      roles,
      config,
      uptimes
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      let botdata = await botsdata.findOne({
        privateURL: req.params.privateURL
      });
      let x = await votes.findOne({ user: req.user.id, bot: botdata.botID });
      if (x)
        return res.redirect(
          "/error?code=400&message=You can vote every 12 hours."
        );
      await votes.findOneAndUpdate(
        { bot: botdata.botID, user: req.user.id },
      ...
      client.channels.cache
        .get(channels.votes)
        .send(
          `**${req.user.username}** voted **${
            botdata.username
          }** **\`(${botdata.votes + 1} votes)\`**`
        );
      return res.redirect(
        `/bot/@${req.params.privateURL}/vote?success=true&message=You voted successfully. You can vote again after 12 hours.`
      );
      renderTemplate(res, req, "botlist/vote.ejs", {
        req,
        roles,
  ...
  app.get("/bot/:botID/vote", checkMaintence, async (req, res) => {
    ...
    if (botdata.privateURL)
      return res.redirect("/bot/@" + botdata.privateURL + "/vote");
    if (!botdata)
      return res.redirect(
        "/error?code=404&message=You entered an invalid bot id."
      );
  ...
  app.get("/bot/:botID/edit", checkMaintence, checkAuth, async (req, res) => {
    ...
    if (!botdata)
      return res.redirect(
        "/error?code=404&message=Introdujiste una id de bot invalida."
      );
    if (
      req.user.id == botdata.ownerID ||
      botdata.coowners.includes(req.user.id)
      ...
      res.redirect(
        "/error?code=404&message=Pará editar este bot, debes ser uno de sus creadores."
      );
    }
  });
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      client.channels.cache
        .get(channels.botlog)
        .send(
          `User **${botdata.ownerName}** requested a certificate for bot **${botdata.username}**.`
        );
    }
  );
  //
    ...
    async (req, res) => {
      ...
      client.users.fetch(req.params.botID).then(bot => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> The bot **${bot.tag}** has been approved. `
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(`Your bot **${bot.tag}** has been approved.`);
      });
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> your bot **${botdata.username}** has been declined. `
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(
            `Your bot **${botdata.username}** has been declined.\nReason: **${
              rBody["reason"]
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.botID).then(bot => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> bot **${bot.tag}** has been granted a certificate.`
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(`Your bot **${bot.tag}** has been certified.`);
      });
      ...
      guild.members.cache
        .get(botdata.ownerID)
        .roles.add(roles.certified_developer);
      if (botdata.coowners) {
        botdata.coowners.map(a => {
    ...
    async (req, res) => {
      ...
      client.users.fetch(botdata.botID).then(bot => {
        client.channels.cache
          .get(channels.botlog)
          .send(
            `<@${botdata.ownerID}> bot **${bot.tag}** has not been granted a certificate.`
          );
        client.users.cache
          .get(botdata.ownerID)
          .send(
            `Your bot **${
              bot.tag
            }** certificate application has been declined.\nReason: **${
              rBody["reason"]
            }**`
          );
      });
      ...
      guild.members.cache
        .get(botdata.ownerID)
        .roles.remove(roles.certified_developer);
      return res.redirect(
  ...
  app.get(
    ...
    checkAuth,
    async (req, res) => {
      if (!settingsc.owner.includes(req.user.id))
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      let bakimdata = await maintenceSchema.findOne({
        server: settingsc.serverID
      });
      if (bakimdata)
        return res.redirect(
          "../admin/maintence?error=true&message=Maintenance mode has already been activated for this site."
        );
      client.channels.cache
        .get(channels.webstatus)
        .send(
          `<a:mantenimiento:853759884121473034> DiscordSpace has been switched to __maintance__ due to **${req.body.reason}**`
        )
        .then(a => {
          new maintenceSchema({
            server: settingsc.serverID,
      ...
      return res.redirect(
        "../admin/maintence?success=true&message=Maintence opened."
      );
    }
  );
  app.post(
    ...
    async (req, res) => {
      ...
      if (!bakimdata)
        return res.redirect(
          "../admin/maintence?error=true&message=The website is not in maintenance mode anyway."
        );
      const bakimsonaerdikardesvcodes = new dc.MessageEmbed()
        .setAuthor("discordspacelist.ga", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(
          `<a:lluvia_de_dinero:838902567420428318> DiscordspaceList are **active** again!\n[Click to redirect website](https://discordspacelist.ga)`
        )
        .setFooter("Discordspace © All rights reserved.");
      await client.channels.cache
        .get(channels.webstatus)
        .messages.fetch(bakimdata.bakimmsg)
        .then(a => {
          a.edit(
            `~~ <a:maintence:833375738547535913> DiscordSpacelist has been switched to __maintance__ due to **${bakimdata.reason}** ~~`,
      ...
      client.channels.cache
        ...
        .send(".")
        .then(b => {
          b.delete({ timeout: 500 });
        });
      await maintenceSchema.deleteOne({ server: settingsc.serverID }, function(
        error,
        server
      ) {
      ...
      return res.redirect(
        "../admin/maintence?success=true&message=Maintenance mode has been shut down successfully."
      );
    }
  );
  app.get(
  ...
  app.get("/api/:botID/voted/:userID", async (req, res) => {
    ...
    res.header("Content-Type", "application/json");
    if (botdata) {
      res.json({ user: req.params.userID, hasvote: "yes" });
    } else {
      res.json({ user: req.params.userID, hasvote: "no" });
    }
  });
